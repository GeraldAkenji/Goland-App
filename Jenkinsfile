pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "geraldakenji/golang-app:v-0.0${env.BUILD_NUMBER}-stable"
    }
    stages {
       ...
       stage("Git Checkout") {
            steps {
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/GeraldAkenji/Goland-App.git']])
            }
       }
       stage("Docker Login") {
            steps {
